...
import '../../oauth_browser_flow.dart';
...
class DropboxProvider implements StorageProvider {
  ...
  DropboxProvider(this.profile, {this.clientId, this.clientSecret});
  ...
  Future<void> connect() async {
    ...
    if (clientId == null || clientId!.isEmpty) {
      throw Exception(
        'Missing Client ID. Please configure API Keys in Settings.',
      );
    }
    ...
    _client = await OAuthBrowserFlow.authorize(grant: grant);
    _isConnected = true;
  }
  ...
  Future<List<FileEntry>> list(String path, [ListOptions? options]) async {
    ...
    return entries.map((e) {
      ...
      return FileEntry(
        ...
        modified: isDir
            ? DateTime.now()
            : DateTime.parse(e['client_modified'] as String),
        permissions: '',
      );
    }).toList();
  }
  ...
  Future<void> rename(String oldPath, String newName) async {
    ...
    final response = await _client!.post(
      ...
      headers: {'Content-Type': 'application/json'},
      body: jsonEncode({'from_path': oldPath, 'to_path': newPath}),
    );
